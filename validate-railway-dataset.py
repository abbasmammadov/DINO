...
from engine_vis import evaluate, train_one_epoch, test
import models
...
def main(args):
    ...
    with open(os.path.join(args.coco_path, "data.yaml"), 'w') as f:
        f.write("path: " + args.coco_path)
        f.write("\n\n")
        f.write("train: train")
        f.write("\n")
        f.write("val: val")
    ...
    for mode in ["test"]:
        ...
        mypath = args.coco_path + "/" + mode + "/"
        mypath_label = args.coco_path + "/" + mode + "_json/"
        #print(mypath, mypath_label)
        os.makedirs(args.coco_path + "/annotations/", exist_ok=True)
        ...
        with open(args.coco_path + '/annotations/instances_'+ mode +'.json', "w", encoding='utf-8') as outfile:
